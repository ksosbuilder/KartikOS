...
- [x] Decide stack (web app first; PRD's cross-platform/offline goals are
      long-term, not MVP — see "Technical Architecture" section for the
      full ambition, but don't build for it yet)
- [x] Recommended default: Next.js + TypeScript + Postgres (via Supabase or
...
- [x] Repo scaffold, linting, CI, deploy target
- [x] `CLAUDE.md` written (stack, conventions, folder layout)
- [x] Auth (single user is fine for MVP — PRD explicitly says architecture
      should just _not block_ multi-user later, not support it now)
...
This phase makes the app _useful_ before it's _alive_.
...
- [ ] Which analytics/chassis get a real algorithm vs. a simple placeholder
      ...
      _philosophy_ more than _formula_ — formulas need to be defined)
